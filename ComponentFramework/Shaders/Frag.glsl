...
vec3 SDFPreOpRepetition( in vec3 p, in vec3 c )
{
    return mod(p+0.5*c,c)-0.5*c;
}

vec3 SDFPreOpRepeitionClamp( in vec3 p, in float c, in vec3 l )
{
    return p-c*clamp(round(p/c),-l,l);
}

float displacement(vec3 p, in float a)
{
	return sin(a * p.x) * sin(a * p.y) * sin(a * p.z);
}

float SDFOpDisplace(in float prim, in vec3 p, in float amount)
{
	return prim + displacement(p, amount);
}

vec3 SDFPreOpTwist(in vec3 p, in float k)
{
	float c = cos(k * p.y);
	float s = sin(k * p.y);
	mat2 m = mat2(c, -s, s, c);
...
vec3 SDFPreOpBend(in vec3 p, in float k)
{
	float c = cos(k * p.x);
	float s = sin(k * p.x);
	mat2 m = mat2(c, -s, s, c);
...
void main ()
{
	...
	vec3 color = vec3(0.1, 1.0, 1.0) * max(0.0, dot(norm, normalize(light - p)));
	float specular = pow(max(0.0, dot(reflection, normalize(light - p))), 14.0);
	vec3 ambient = vec3(0.5, 0.3, 0.1);
